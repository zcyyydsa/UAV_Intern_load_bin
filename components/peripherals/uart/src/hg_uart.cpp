...
int HgUart::uart_open(const char* dev, int speed)
{
    int fd;
...
void HgUart::uart_close(int fd)
{
    close(fd);
}

int HgUart::uart_set_opt(int fd, int nSpeed, int nBits, char nEvent, int nStop)
{
...
int HgUart::uart_send(int fd, const unsigned char* buf, int len)
{
    int ret = 0;
    int count = 0;
...
int HgUart::uart_recv(int fd, char* buf, int len, int timeout_ms)
{
    fd_set rset;
    int ret;
    struct timeval t;
